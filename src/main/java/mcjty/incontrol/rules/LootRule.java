...
public class LootRule extends RuleBase<RuleBase.EventGetter> {
    ...
    private final GenericRuleEvaluator<LivingDropsEvent> ruleEvaluator;
    ...
    protected void addActions(AttributeMap map, IModRuleCompatibilityLayer layer) {
        ...
        if(map.has(ACTION_ITEM)) addItem(map);
        if(map.has(ACTION_REMOVE)) removeItem(map);
        if(map.has(ACTION_REMOVEALL)) removeAll = map.get(ACTION_REMOVEALL);
    }
    ...
    private Function<Integer, Integer> getCountFunction(@Nullable String itemcount) {
        if(itemcount == null) return looting -> 1;
        ...
        if(loottable.length == 1) {
            // Easy case
            if(min[0] == max[0]) return looting -> min[0];
            else return looting -> rnd.nextInt(max[0] - min[0] + 1) + min[0];
        }
        else {
            return looting -> {
                if(looting >= min.length) return rnd.nextInt(max[min.length - 1] - min[min.length - 1] + 1) + min[min.length - 1];
                else if(looting >= 0) return rnd.nextInt(max[looting] - min[looting] + 1) + min[looting];
                else return rnd.nextInt(max[0] - min[0] + 1) + min[0];
            };
        }
    }
    ...
    private List<Pair<ItemStack, Function<Integer, Integer>>> getItems(List<String> itemNames, @Nullable String nbtJson, @Nullable String itemcount) {
        ...
        for(String name : itemNames) {
            ...
            if(stack.isEmpty()) InControl.setup.getLogger().log(Level.ERROR, "Unknown item '{}'!", name);
            else {
                if(nbtJson != null) {
                    try {
    ...
    private void removeItem(AttributeMap map) {
        toRemoveItems.addAll(GenericRuleEvaluator.getItems(map.getList(ACTION_REMOVE), logger));
    }
